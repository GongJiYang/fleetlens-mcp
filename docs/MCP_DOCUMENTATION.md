...
- Antigravity CLI does not load Codex skills directly
...
- Global MCP servers are loaded from `~/.gemini/config/mcp_config.json`
- Do not add MCP servers to `~/.gemini/antigravity-cli/settings.json`; that file stores CLI preferences and the MCP screen may show `No MCP servers configured`
...
Then create or update `~/.gemini/config/mcp_config.json`, preserving any existing servers:
...
For a remote connection, start `luminon start remote` and configure the endpoint in the same file with `serverUrl` and the Bearer token generated by `luminon token create`:

```json
{
  "mcpServers": {
    "luminon_remote": {
      "serverUrl": "<myluminonserver>",
      "headers": {
        "Authorization": "Bearer <lbt_token>"
      }
    }
  }
}
